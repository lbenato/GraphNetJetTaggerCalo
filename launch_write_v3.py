...
in_folder  = '/nfs/dust/cms/group/cms-llp/v3_calo_AOD_2018_unmerged_v3/'
out_folder = '/nfs/dust/cms/group/cms-llp/dataframes_lisa/v3_calo_AOD_2018/'#out dir for write_

in_convert = '/nfs/dust/cms/group/cms-llp/dataframes_lisa/v3_calo_AOD_2018/'
out_convert = '/nfs/dust/cms/group/cms-llp/dataframes_lisa/v3_calo_AOD_2018_dnn/'
...
def do_write(in_folder, out_folder, cols=var_list_tree):
    for a in sgn+bkg:
        for i, s in enumerate(samples[a]['files']):
            ...
            if not(os.path.exists('/nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_write_v3/'+s)):
                os.mkdir('/nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_write_v3/'+s)
            ...
            for n in range(max_loop):
                os.chdir('/nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_write_v3/'+s+'/')
                print("Loop n. ", n)
                print(root_files[n])
                #for local test:
                #write_h5_v3(IN,OUT,root_files[n],xs,LUMI,cols,tree_name="skim",counter_hist="c_nEvents",sel_cut="")
                #exit()
                #write python macro
                ...
                with open('job_write_'+str(n)+'.sh', 'w') as fout:
                    ...
                    fout.write('python /nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_write_v3/'+s+'/write_macro_'+str(n)+'.py'  +' \n')
                os.system('chmod 755 job_write_'+str(n)+'.sh')
                ###os.system('sh job_skim_'+str(n)+'.sh')
        
                #write submit config
                with open('submit_write_'+str(n)+'.submit', 'w') as fout:
                    fout.write('executable   = /nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_write_v3/'+s+'/job_write_'+ str(n) + '.sh \n')
                    fout.write('output       = /nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_write_v3/'+s+'/out_write_'+ str(n) + '.txt \n')
                    fout.write('error        = /nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_write_v3/'+s+'/error_write_'+ str(n) + '.txt \n')
                    fout.write('log          = /nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_write_v3/'+s+'/log_write_'+ str(n) + '.txt \n')
                    fout.write(' \n')
                ...
                ##submit condor
                os.chdir('../../.')
                os.system('condor_submit /nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_write_v3/'+s+'/submit_write_'+str(n)+'.submit' + ' \n')
...
def do_convert(inp,out,nj,npf,cols):
    for a in sgn+bkg:
        for i, s in enumerate(samples[a]['files']):
            ...
            print("Prepare condor submission scripts")
            if not(os.path.exists('/nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_conv_v3/'+s)):
                os.mkdir('/nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_conv_v3/'+s)
            ...
            #for n, f in enumerate([skim_list[0]]):
                ...
                os.chdir('/nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_conv_v3/'+s+'/')
                print("Loop n. ", n)
                print(f)
                ...
                with open('job_convert_'+str(n)+'.sh', 'w') as fout:
                    ...
                    fout.write('cd /nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/ \n')
                    fout.write('source activate /nfs/dust/cms/user/lbenato/anaconda2/envs/particlenet \n')
                    fout.write('python /nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_conv_v3/'+s+'/convert_macro_'+str(n)+'.py'  +' \n')
                ...
                with open('submit_convert_'+str(n)+'.submit', 'w') as fout:
                    fout.write('executable   = /nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_conv_v3/'+s+'/job_convert_'+ str(n) + '.sh \n')
                    fout.write('output       = /nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_conv_v3/'+s+'/out_convert_'+ str(n) + '.txt \n')
                    fout.write('error        = /nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_conv_v3/'+s+'/error_convert_'+ str(n) + '.txt \n')
                    fout.write('log          = /nfs/dust/cms/user/lbenato/ML_LLP/GraphNetJetTaggerCalo/condor_conv_v3/'+s+'/log_convert_'+ str(n) + '.txt \n')
                    fout.write(' \n')
                    fout.write('#Requirements = OpSysAndVer == "CentOS7" \n')
                    fout.write('##Requirements = OpSysAndVer == "CentOS7" && CUDADeviceName == "GeForce GTX 1080 Ti" \n')
                    fout.write('#Request_GPUs = 1 \n')
                ...
                ##submit condor
                os.chdir('../../.')
                os.system('condor_submit condor_conv_v3/'+s+'/submit_convert_'+str(n)+'.submit' + ' \n')
    ...
    for a in sgn+bkg:
        ...
        for i, s in enumerate(samples[a]['files']):
            ...
            for n in range(max_loop):
                ...
                #new preselections: HCAL acceptance, and ask valid ECAL/HCAL rec hits
                df_list.append( df[(df["Jet_pt"]>0) & (df["Jet_index"]<max_jetindex) & (df["EventWeight"]>0)  & (df["Jet_eta"]<1.48) & (df["Jet_eta"]>-1.48) & (df["Jet_timeRecHitsEB"]>-100.) & (df["Jet_timeRecHitsHB"]>-100.)] )
                if(n % 100 == 0):
                    print("  * * * * * * * * * * * * * * * * * * * * * * *")
...
def do_mix_signal(type_dataset,folder,features,upsample_factor=0):
    ...
    #Shuffle now, just in case
    df_s = df_s.sample(frac=1).reset_index(drop=True)
    print("Shuffling: ", df_s.shape)
...
def do_mix_background(type_dataset,folder,features):
    ...
    #Shuffle now if you want to use only a percentage of background!
    df_b = df_b.sample(frac=1).reset_index(drop=True)
    print("Shuffling: ", df_b)
...
OUT_NEW_PRESEL = '/nfs/dust/cms/group/cms-llp/dataframes_lisa/v3_calo_AOD_2018_dnn_new_presel/'
